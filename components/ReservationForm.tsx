...
export default function ReservationForm() {
  ...
  return (
    <section id="reserver" className="mx-auto w-full max-w-[1280px] scroll-mt-24 px-5 py-20 md:px-8 md:py-28">
      <div className="mx-auto max-w-xl rounded-[2rem] border border-cocoa/10 bg-milk p-8 md:p-10">
        ...
        {sent ? (
          ...
          <form onSubmit={submit} className="mt-8 grid gap-4">
            <input required placeholder="Votre nom" value={form.name}
              onChange={(e) => setForm({ ...form, name: e.target.value })}
              className="rounded-xl border border-cocoa/20 bg-cream px-4 py-3 outline-none focus:border-blush" />
            <div className="grid grid-cols-3 gap-4">
              <select value={form.people} onChange={(e) => setForm({ ...form, people: e.target.value })}
                className="rounded-xl border border-cocoa/20 bg-cream px-3 py-3 outline-none focus:border-blush">
